...
# 链码的安装，使用 peer chaincode install把链码打包，上传到区块链的某一个节点上
installChaincode () {
	PEER=$1
	setGlobals $PEER
...
# 链码的初始化，对应chaincode_example02中init方法。执行效果是在区块链上初始化了两个账户，分别打了100,200元
...
chaincodeQuery () {
  ...
  do
     ...
     # 查询的是a账户，所以日志里打出来的是90。可以把a改成b，查出结果是210
...
#Query on chaincode on Peer0/Org1, 要是上面查询时传的是b，这里需要改成chaincodeQuery 0 200，这步是做判断，如果不对的话不会往下一步走
...
#Query on chaincode on Peer3/Org2, check if the result is 90，要是上面查询时传的是b，这里需要改成chaincodeQuery 3 210
